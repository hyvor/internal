...
Mail templates extend `templates/mail/mail.html.twig`, which wraps your content with the HYVOR brand header and
footer. Reusable pieces (e.g. buttons) are available as Twig components under `src/Mail/Component`.

```twig
{# templates/mail/approval.html.twig #}
{% extends '@Internal/mail/mail.html.twig' %}

{% block title %}Your post was approved{% endblock %}
{% block heading %}Your post was approved{% endblock %}

{% block content %}
    <p>Hi {{ name }},</p>
    <p>Your post has been approved and is now live.</p>

    <twig:mail:button href="{{ buttonUrl }}">
        View Post
    </twig:mail:button>
{% endblock %}
```

Render it like any other Twig template:

```php
$html = $twig->render('mail/approval.html.twig', [
    'component' => 'post', // used for the brand icon/name in the header
    'name' => $user->name,
    'buttonUrl' => $url,
]);
```
